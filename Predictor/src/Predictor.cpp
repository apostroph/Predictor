...
Predictor::Predictor(int argc, char * argv[]):
 ...
 */
void Predictor::mapInput(double value, int index, int setTarger){
	if(std::isnan(value)){
		//If no input, the input is NULL and tha target is NaN (no learning)
		nextInput(index) = value;
		if(setTarger != -1)
			nextTarget(index) = setTarger;
		else
			nextTarget(index) = -1;
	}else{	
 ...
 */
void Predictor::openFile(string src, int iter, bool train_test){
		...
		{	
			...
			while ( getline (inFile1,line) )
			{
				...
				while(line.find(" ") != std::string::npos){
					...
					mapInput(value, count, output);
					
					line = line.substr(line.find(" ")+1);			
					count++;
				}
				value = string_to_double(line);
				value = (value-221.083)/(63.266);
				mapInput(value, count, output);
			...
			cerr<<"Error while opening input fileat:"<<endl;
			cerr<<"\t"<<src<<endl;
		}
		resetNodes();
	}
